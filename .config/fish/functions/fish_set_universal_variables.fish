function fish_set_universal_variables
    ...
    set -U FZF_ALT_C_COMMAND "find \( -wholename '*.config/*' -or \( -wholename '*.local/share/*' -and -not -wholename '*.local/share/Steam/*' \) \) -or -not -path '*/\.*' 2> /dev/null"
    set -U FZF_CTRL_T_COMMAND "find \( -wholename '*.config/*' \) -or -not -path '*/\.*' 2> /dev/null"
